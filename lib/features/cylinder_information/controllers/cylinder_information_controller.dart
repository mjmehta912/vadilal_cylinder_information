...
class CylinderInformationController extends GetxController {
  ...
  var cylinderStatus = Rx<CylinderStatusDm?>(null);
  ...
  Future<void> fetchCylinderStatus(String cylinderNo) async {
    isLoading(true);
    _snackbarShown = false;

    try {
      final fetchedData =
          await CylinderInformationApiService.fetchCylinderStatus(cylinderNo);

      if (fetchedData != null) {
        cylinderStatus.value = fetchedData;
